...
# ----------------------------------------
# IMPORTANT: Create schema and tables in the 'crypto' database, not 'airflow'
psql -v ON_ERROR_STOP=1 --username "$POSTGRES_USER" --dbname "crypto" <<-EOSQL
    ...
    -- Create Table
    CREATE TABLE IF NOT EXISTS crypto_data.crypto_klines(
    ...
    -- Create Indices
    CREATE INDEX IF NOT EXISTS idx_crypto_klines_symbol_time 
        ON crypto_data.crypto_klines(symbol, open_time DESC);
    CREATE INDEX IF NOT EXISTS idx_crypto_klines_source 
        ON crypto_data.crypto_klines(source_region);
EOSQL
